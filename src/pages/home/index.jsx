...
import {
  ...
  GET_PRODUCTS_HOT_DEALS,
  GET_PRODUCTS_TOP_SELLING,
} from "../../constants/constants";

const HomePage = () => {
  useScrollTop();
  const hotDeal = useFetchApi({ URL: GET_PRODUCTS_HOT_DEALS });
  const topSelling = useFetchApi({ URL: GET_PRODUCTS_TOP_SELLING });
  const featured = useFetchApi({ URL: GET_PRODUCTS_FEATURED });
  const recomended = useFetchApi({ URL: GET_PRODUCTS });
  const post = useFetchApi({ URL: GET_POST });
  ...
  return (
    <div className="bg-[#F1F5F6]">
      <div className="max-w-[1410px] relative px-5 py-5 mx-auto ">
        ...
        <Hotdeals data={hotDeal?.data?.data?.data} loading={hotDeal?.loading} />
        <FeaturedProducts
        ...
        <TopSellingProducts
          data={topSelling.data?.data?.data}
